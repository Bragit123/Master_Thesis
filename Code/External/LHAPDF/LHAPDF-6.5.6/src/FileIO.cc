  ...
  bool File<FILETYPE>::open() {
      ...
      if (is) {
        ...
        if (it != lhapdf_filecontents.end()) {
          ...
          if (world_rank) { // Note: _non_-zero, i.e. all but the head rank get their content from rank 0
            int fsize;
          ...
          } else { // Head rank, actually does the reading
            ...
            (*_streamptr) << infile.rdbuf(); //< the actual file-reading
