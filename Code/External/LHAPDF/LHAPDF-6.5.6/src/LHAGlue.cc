...
// Copyright (C) 2012-2025 The LHAPDF collaboration (see AUTHORS for details)
//
...
extern "C" {
  ...
  void lhapdf_xfxq2_stdpartons_(const int& nset, const int& nmem, const double& x, const double& q2, double* xfs) {
    ...
    std::vector<double> data(13, 0.0); // Pre-allocate and zero return memory
    try {
      ACTIVESETS[nset].activeMember()->xfxQ2(x, q2, data);
    } catch (const std::exception& e) {
      std::fill_n(xfs, 13, 0.0); // Zero output on error
      CURRENTSET = nset;
      return;
    }
    std::copy(data.begin(), data.end(), xfs); // Fast copy
    CURRENTSET = nset;
  }
  ...
  void initpdfsetbynamem_(const int& nset, const char* setname, int setnamelength) {
    ...
    string p = setname;
    p.erase(std::max(0,std::min(setnamelength, (int)p.length())), std::string::npos);
    // Strip file extension for backward compatibility
  ...
  void evolvepdfm_(const int& nset, const double& x, const double& q, double* fxq) {
    ...
    std::vector<double> data(13, 0.0); // Pre-allocate and zero return memory
    try {
      ACTIVESETS[nset].activeMember()->xfxQ(x, q, data);
    } catch (const std::exception& e) {
      std::fill_n(fxq, 13, 0.0); // Zero output on error
      CURRENTSET = nset;
      return;
    }
    std::copy(data.begin(), data.end(), fxq); // Fast copy
    // Update current set focus
    CURRENTSET = nset;
  }
