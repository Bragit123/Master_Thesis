...
namespace LHAPDF {
  ...
  size_t PDFErrInfo::nmemCore() const {
    return qparts[0][0].second; //< first part is core, and has only one env component
  }

  size_t PDFErrInfo::nmemPar() const {
    size_t rtn = 0;
    for (size_t i = 1; i < qparts.size(); ++i) { //< note start at 1 to skip core...
      for (size_t j = 0; j < qparts[i].size(); ++j) { //< ...but incl all the par-parts' env components
  ...
  void PDFSet::uncertainty(PDFUncertainty& rtn, const vector<double>& values, double cl, bool alternative) const {
    ...
    size_t nmem = errinfo.nmemCore(); ///< @note The number of members in the error set, not the PDF set (which also has the nominal)
    if (nmem <= 0)
    ...
    if (!in_closed_range(reqCL, 0, 1)) // || !in_range(setCL, 0, 1))
      throw UserError("Error in LHAPDF::PDFSet::uncertainty. Requested confidence level outside [0,1] range.");
    ...
    if (errinfo.coreType() == "replicas") {
      ...
      if (alternative) {
        ...
        copy(values.begin()+1, values.begin()+1+nmem, sorted.begin()); //< only copy the error set, no +1 offset from now
        sort(sorted.begin(), sorted.end());
        // Define central value to be median.
        if (nmem % 2 != 0) { // odd nmem => one middle value, also works for nmem = 1
          rtn.central = sorted[nmem/2];
        } else { // even nmem => average of two middle values
          rtn.central = 0.5*(sorted[nmem/2 - 1] + sorted[nmem/2]);
        }
        // Define uncertainties via quantiles with a CL given by reqCL.
        /// @todo Walk in from the lower/upper edges to avoid central-member ambiguities
        const double pedge = 0.5*(1-reqCL); //< probability required on each tail, in [0, 0.5]
        const double fiq = pedge*(nmem-1); //< "floating-point index" of quantile offset from each edge, for n-1 "gaps" between sorted replica values
        double _i; const double fiqfrac = modf(fiq, &_i); //< fractional part of fiq, in [0,0.999..]
        /// @note This should not be in a hot loop, hence not specialised for super-rare integer fiq's
        const int iqlow = floor(fiq), iqhigh = ceil(fiq); //< indices of replicas surrounding fiq
        //cerr << "(" << iqlow << ", " << iqhigh << "; " << nmem-1-iqlow << ", " << nmem-1-iqhigh << ")" << endl;
        const double lower = (1-fiqfrac)*sorted[iqlow] + fiqfrac*sorted[iqhigh]; //< weighted average of replicas surrounding Q = CL/2
        const double upper = (1-fiqfrac)*sorted[nmem-1-iqlow] + fiqfrac*sorted[nmem-1-iqhigh]; //< weighted average of replicas surrounding Q = 1 - CL/2
        rtn.errplus = upper - rtn.central;
        rtn.errminus = rtn.central - lower;
        rtn.errsymm = (rtn.errplus + rtn.errminus)/2.0; // average of up and down errors
  ...
  void PDFSet::_checkPdfType(const vector<string>& pdftypes) const {
    if (pdftypes.size() != size())
      throw UserError("Error in LHAPDF::PDFSet::_checkPdfType. Input vector must contain values for all PDF members.");
    const PDFErrInfo errinfo = errorInfo();

    // Check that zeroth member has "MemType: central".
    if (pdftypes[0] != "central")
      throw MetadataError("Member 0, type = " + pdftypes[0] + " should be set to \"MemType: central\".");

    // Check that PDF-error members have "MemType: replica" or "MemType: error".
    string exptype = "";
    if (errinfo.coreType() == "replicas") {
      exptype = "replica";
      ...
      throw MetadataError("\"ErrorType: " + errorType() + "\" not supported by LHAPDF::PDFSet::_checkPdfType.");
    }
    for (size_t imem = 1; imem < errinfo.nmemCore()+1; imem++) {
      if (pdftypes[imem] != exptype) {
        throw MetadataError("Member " + to_str(imem) + ", type = " + pdftypes[imem] + " should be \"MemType: " + exptype + "\".");
      }
    }

    // Check that possible parameter variations have "MemType: central" or "MemType: variation".
    for (size_t imem = errinfo.nmemCore()+1; imem < size(); imem++) {
      if (pdftypes[imem] != "central" && pdftypes[imem] != "variation") {
        throw MetadataError("Member " + to_str(imem) + ", type = " + pdftypes[imem] + " should be \"MemType: central\" or \"MemType: variation\".");
      }
    }

    //cout << "Success: type of each member matches the ErrorType of the set." << endl;
  }
