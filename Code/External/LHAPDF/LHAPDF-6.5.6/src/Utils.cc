...
namespace LHAPDF {
  ...
  bool file_exists(const std::string& file,int mode) {
    ...
    #ifdef HAVE_MPI
    int world_rank = -1;
    if (mode!=1) MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    if (mode==1 || world_rank == 0) {
      struct stat fst;
      if (stat(file.c_str(), &fst) != -1) exists = (fst.st_mode & S_IFMT) == S_IFREG;
    }
    if (mode!=1) MPI_Bcast(&exists,1,MPI_INT,0,MPI_COMM_WORLD);
    #else
    struct stat fst;
    if (stat(file.c_str(), &fst) != -1) exists = (fst.st_mode & S_IFMT) == S_IFREG;
    #endif
  ...
  namespace {
    ...
    /// @brief gamma functions from Cephes library -- http://www.netlib.org/cephes
    ///
  ...
  double norm_quantile(double p) {
    ...
    if  (p == 0 || p == 1) return 0; //< not sure why, but this works...
