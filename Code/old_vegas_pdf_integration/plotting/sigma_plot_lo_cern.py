...
for sid in slepton_ids:
  try:
    filename = "xsec_mass_" + str(sid) + ".dat"
    filepath = OUTPUT_DIR/filename
    m, sigma, nlo, hadron_side, slepton_side = np.loadtxt(filepath, unpack=True)
  except:
...
for m, sigma, color, style, label in zip(all_mass_arrs, all_sigma_arrs, colors, line_styles, slepton_labels):
  plt.plot(m, sigma, color=color, linestyle=style, marker="o", label=label)

plt.yscale("log")
plt.tight_layout()
plt.legend()
plt.savefig(PLOT_DIR / "cern_lo_xsec.pdf")
...
for m_arr, ratio, color, style, label in zip(mass_arrs, ratio, colors, line_styles, slepton_labels):
  plt.plot(m, ratio, color=color, linestyle=style, marker="o", label=label)

plt.tight_layout()
plt.legend()
plt.savefig(PLOT_DIR / "cern_ratio_xsec.pdf")