...
//   double weight = self->w_LO(1.0, Q2);
...
//   ff[0] = born * jacobian * xfq * xfqbar / (x1 * tau) * weight;
...
//   const double z = 1.0;
//   const double w_soft = self->w_hadron_soft(z, Q2);
//   const double w_plus_1 = self->w_hadron_plus_1(z, Q2);
//   const double w_plus_log = self->w_hadron_plus_log(z, Q2);
...
//   // Factor 2 to account for changing which particle is (anti-)quark
//   ff[0] = born * jacobian * xfq * xfqbar / (x1 * tau) * (w_soft + w_plus_1*F1 + w_plus_log*Flog);
...
//   const double w_rad = self->w_hadron_rad(z, Q2);
//   const double w_plus_1 = self->w_hadron_plus_1(z, Q2);
//   const double w_plus_log = self->w_hadron_plus_log(z, Q2);
//   const double w_plus_1_one = self->w_hadron_plus_1(1.0, Q2);
//   const double w_plus_log_one = self->w_hadron_plus_log(1.0, Q2);
...
//   // Factor 2 to account for changing which particle is (anti-)quark
//   ff[0] = born * jacobian * xfq / (x1*x1 * x2*x2) * (rad_term + f_term);
...
//   const double z = 1.0;
//   const double w_soft = self->w_slepton_soft(z, Q2);
//   const double w_plus_1 = self->w_slepton_plus_1(z, Q2);
//   const double F1 = log(1.0 - tau/x1);
...
//   // Factor 2 to account for changing which particle is (anti-)quark
//   ff[0] = born * jacobian * xfq * xfqbar / (x1 * tau) * (w_soft + w_plus_1*F1);
...
//   const double z = tau/(x1*x2);
//   const double w_rad = self->w_slepton_rad(z, Q2);
//   const double w_plus_1 = self->w_slepton_plus_1(z, Q2);
//   const double w_plus_1_one = self->w_slepton_plus_1(1.0, Q2);
...
int CrossSection::integrand_LO_x1_Q(
  ...
  // ATTEMPT AT LOGARITHMIC VERSION
  CrossSection* self = static_cast<CrossSection*>(userdata); // Necessary to keep class variables while being compatible with Vegas()
  double Q2_min = self->Q2_min;
  double Q2_max = self->Q2_max;
  double s = self->s;
...
int CrossSection::integrand_hadron_x1_x2_Q(
  ...
  double f1 = 1.0 / (1.0 - z);
  double flog = log(1.0 - z) / (1.0 - z);

  if (f1_term < tol) {
    std::cout << "f1: " << f1_term << " | " << f1 << " || " << f1_term*f1 << std::endl;
    f1 = 0.0;
  }
  if (flog_term < tol) {
    std::cout << "flog: " << flog_term << " | " << flog << " || " << flog_term*flog << std::endl;
    flog = 0.0;
  }
...
double CrossSection::I_virt_real(double z, double Q2) {
  const double m12 = mB*mB;
  const double m22 = mA*mA;
  const double s_parton = Q2/z;
  const double lambda_inv = 1.0 / Utils::Kallen(s_parton, m12, m22);
  ...
  const std::complex<double> term1 = -(s_parton - m_sum) * Csm12;
  const std::complex<double> term2 = 0.5 * Bm1 + 0.5 * Bm2;
  const std::complex<double> term3 = - 2.*s_parton*(s_parton - m_sum)*lambda_inv * Bsm12;
  const std::complex<double> term4 = 2.*m12*(s_parton - m_diff)*lambda_inv * Bm1;
  const std::complex<double> term5 = 2.*m22*(s_parton + m_diff)*lambda_inv * Bm2;

  const double res = Re(term1 + term2 + term3 + term4 + term5);
  return res;
}
double CrossSection::I_emission_1(double Q2) {
  const double mt12 = mB*mB/Q2;
  const double mt22 = mA*mA/Q2;
  const double mt_1_sum = 1. - mt12 - mt22;
  const double lambda_sqrt = sqrt(Utils::Kallen(1., mt12, mt22));

  const double term1 = -mt_1_sum * log((mt_1_sum - lambda_sqrt)/(2. * sqrt(mt12*mt22)));
  const double term2 = -lambda_sqrt;
  return term1 + term2;
}
double CrossSection::I_emission_2(double Q2) {
  const double mt12 = mB*mB/Q2;
  const double mt22 = mA*mA/Q2;
  ...
  const double tol = 1e-10;
  const double bracket_dilog_Im = Im(bracket_dilog);
  const double bracket_dilog_Re = Re(bracket_dilog);

  if (std::abs(bracket_dilog_Im) > tol) {
    std::cout << "Warning: non-zero imaginary part encountered in I_emission_2:\n"
              << "    bracket_dilog_Im = " << bracket_dilog_Im << std::endl;
  }

  const double log_xpxm = log((1.+xp)/(1.+xm));
  const double log_mxm = log(1.-xm);
  const double log_pxm = log(1.+xm);
  const double bracket_no_dilog = -M_PI*M_PI/3. + 0.5*log_xpxm*log_xpxm + log_mxm*log_mxm
                          - log_pxm*log_pxm + log(2.*mt12*mt22)*log(mt22/mt12);
  const double term1 = (1.-mt12-mt22) * (bracket_dilog_Re + bracket_no_dilog);
  const double log_xm2_xp2 = log((1.-xm*xm)/(1.-xp*xp));
  const double log_xmxp_xmxp = log((1.-xm)*(1.+xp)/((1.+xm)*(1.-xp)));
  const double term2 = -lambda_sqrt * log(4.*lambda)
                      + 0.5*(mt12-mt22)*log_xm2_xp2 + 0.5*log_xmxp_xmxp;
  return term1 + term2;
}

// Weights
double CrossSection::w_LO(double z, double Q2) {
  return 1.0;
}
double CrossSection::w_hadron_soft(double z, double Q2) {
  double Qq;
  if (quark_id % 2 == 0) {
    // up-type quark
    Qq = Const::Qu;
...
double CrossSection::w_hadron_rad(double z, double Q2) {
  double Qq;
  if (quark_id % 2 == 0) {
    // up-type quark
    Qq = Const::Qu;
...
double CrossSection::w_hadron_plus_1(double z, double Q2) {
  double Qq;
  if (quark_id % 2 == 0) {
    // up-type quark
    Qq = Const::Qu;
...
double CrossSection::w_hadron_plus_log(double z, double Q2) {
  double Qq;
  if (quark_id % 2 == 0) {
    // up-type quark
    Qq = Const::Qu;
...
double CrossSection::w_slepton_soft(double z, double Q2) {
  const double mt12 = mB*mB/Q2;
  const double mt22 = mA*mA/Q2;
  const double I1 = I_emission_1(Q2);
  const double I2 = I_emission_2(Q2);
  const double Iv_real = I_virt_real(z, Q2);
  const double lambda = Utils::Kallen(1., mt12, mt22);
  const double lambda_sqrt_inv = 1./sqrt(lambda);
  
  double res = Const::ALPHA * M_1_PI;
  res *= Iv_real - (log(4.*lambda) + log(muF2/Q2))*lambda_sqrt_inv * I1
        + lambda_sqrt_inv * I2;
  return res;
}
double CrossSection::w_slepton_rad(double z, double Q2) {
  const double mt12 = mB*mB/Q2;
  const double mt22 = mA*mA/Q2;
  const double lambda = Utils::Kallen(1., mt12, mt22);
  const double lambda_sqrt = sqrt(lambda);
  ...
  double res = Const::ALPHA * M_1_PI;
  res *= 2. * (1. - z) * lambda_sqrt / (lambda_z * lambda_z_sqrt);
  return res;
}
double CrossSection::w_slepton_plus_1(double z, double Q2) {
  const double mt12 = mB*mB/Q2;
  const double mt22 = mA*mA/Q2;
  const double lambda_z = Utils::Kallen(1., z*mt12, z*mt22);
  const double I1 = I_emission_1(Q2);
...
double CrossSection::full_xsec(
  ...
  std::vector<int> all_quark_ids; // Store antiquarks as well
  for (int quark_id_ : quark_ids) {
    all_quark_ids.push_back(quark_id_);
    all_quark_ids.push_back(-quark_id_);
  }
  
  double xsec = 0.0;
  // for (int quark_id_ : all_quark_ids) {
  //   quark_id = quark_id_;
  //   if (subset == 0) {
...
//     if (subset == 0) {
//       double integral[1], error[1], prob[1];
//       Utils::integrate_vegas(1, 1, integrand_LO_x1, this, epsrel, epsabs,
//                             maxeval, integral, error, prob);
//       dxsec += 2.*integral[0];
//     }
//     // } else if (subset == 1) {
...
// int CrossSection::integrand_LO_x1(
//     const int* ndim,
//     const cubareal xx[],
//     const int* ncomp,
//     cubareal ff[],
...
//   const double x1_end = 1.0;
//   const double jacobian_x1 = x1_end - x1_start;
//   double x1 = x1_start + jacobian_x1 * xx[0];
//   double x2 = tau / x1;
  
//   double weight = self->w_LO(1.0, Q2);
  
//   const double jacobian = jacobian_x1;
