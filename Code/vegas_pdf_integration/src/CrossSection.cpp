...
// Integrands //
////////////////
int CrossSection::integrand_LO_full(
    const int* ndim,
    const cubareal xx[],
    const int* ncomp,
    cubareal ff[],
...
int CrossSection::integrand_hadron_x1_x2_Q(
  ...
  // std::cout << w_rad << " | " << w_plus_1 << " | " << w_plus_log << " | " << w_plus_1_one << " | " << w_plus_log_one << std::endl;
...
double CrossSection::I_virt_real(double z) {
  ...
  const std::complex<double> term2 = 0.5 * B0(m12, 0.0, m12) + 0.5 * B0(m22, 0.0, m22);
  const std::complex<double> term3 = - 2.0*s_hat*(s_hat - m_sum)*lambda_inv * B0(s_hat, m12, m22);
  const std::complex<double> term4 = 2.0*m12*(s_hat - m_diff)*lambda_inv * B0(m12, 0.0, m12);
  const std::complex<double> term5 = 2.0*m22*(s_hat + m_diff)*lambda_inv * B0(m22, 0.0, m22);
...
double CrossSection::I_emission_2() {
  ...
  const std::complex<double> bracket_Li2 = Li2((1.0-xp)/(1.0-xm)) + Li2((1.0+xm)/(1.0+xp));

  const double tol = 1e-10;
  const double bracket_Li2_Im = Im(bracket_Li2);
  const double bracket_Li2_Re = Re(bracket_Li2);
...
double CrossSection::full_xsec(
  ...
  for (int quark_id_ : quark_ids) {
    ...
    if (subset == 0) {
      ...
      Utils::integrate_vegas(2, 1, integrand_LO_full, this, epsrel, epsabs,
      ...
      // std::cout << integral1[0] << " | " << integral2[0] << " | " << integral1[0]+integral2[0] << std::endl;
    }
  }
